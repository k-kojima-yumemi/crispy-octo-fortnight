...
export const BirthdayForm: FC = () => {
  ...
  const handleMonthChange = async (e: Event) => {
    ...
    if (month) {
      try {
        ...
        setError(err instanceof Error ? err.message : '日付の取得に失敗しました');
        setDays([]);
      }
    } else {
      setDays([]);
  ...
  return (
    <form method="post" action="/result" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700">日</label>
        <select
          name="day"
          value={selectedDay}
          onChange={(e) => setSelectedDay((e.target as HTMLSelectElement).value)}
