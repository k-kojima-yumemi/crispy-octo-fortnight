  ...
  useEffect(() => {
    const calculateBirthday = async () => {
      try {
        ...
        setResult(`次の誕生日まであと${data.daysUntilBirthday}日です！`);
      } catch (error) {
        setResult(error instanceof Error ? error.message : '計算中にエラーが発生しました。');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div class="text-center">
      ...
      {isLoading ? (
        ...
        <div class={`text-2xl font-bold ${result.includes('エラー') ? 'text-red-600' : 'text-indigo-600'}`}>
          {result}
        </div>
      )}
      <a
